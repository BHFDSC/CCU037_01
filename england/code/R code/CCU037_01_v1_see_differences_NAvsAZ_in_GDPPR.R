...
mydata1<- dbGetQuery(con,"SELECT NHS_NUMBER_DEID, RAW_ETHNICITY, CATEGORISED_ETHNICITY, SEX, DATE_OF_BIRTH, DATE_OF_DEATH 
  FROM .ccu037_dp_study_population_geography")
...
mydata2<- dbGetQuery(con,"SELECT NHS_NUMBER_DEID,bmi, smoking_status, heavy_alcohol_hasbled  FROM .ccu037_01_study_population_cov")
mydata = left_join(mydata1,mydata2)

#'Load _study_population_como 
mydata1 = mydata #save prior status and merge with following new columns
mydata2<- dbGetQuery(con,"SELECT NHS_NUMBER_DEID, AF, AMI, CKD, COPD, DVT, HF, PE, cancer  
                            FROM .ccu037_01_study_population_como")
...
#eth_codes<- dbGetQuery(con,"SELECT PERSON_ID, ETHNIC FROM .ccu037_gdppr_skinny_patient_char_pcandsnomedcode")
#mydata = left_join(mydata,eth_codes,by=c("NHS_NUMBER_DEID" = "PERSON_ID"))

eth_codes<- dbGetQuery(con,"SELECT PERSON_ID, ETHNIC FROM .ccu037_01_ethnos_codes_for_individuals_gdppr_23032022")
mydata = left_join(mydata,eth_codes,by=c("NHS_NUMBER_DEID" = "PERSON_ID"))
...
# set target folder
setwd("/mnt/efs/marta.pinedamoncusi//CCU037/CCU037_01")
# cohort_index
...
# setwd("/mnt/efs/marta.pinedamoncusi//CCU037/CCU037_01/DATA")
# load(CCU037_base_cohort_for_comparison_tables_2022_03_23.rds)

mydata<- readRDS("~//CCU037/CCU037_01/DATA/CCU037_base_cohort_for_comparison_tables_2022_03_23.rds")
...
eth_codes <- dbGetQuery(con,"SELECT PERSON_ID, PrimaryCode as PCs
  FROM .ccu037_01_individual_snomed_codes_for_individuals_gdppr_23032022")
...
#cairo_pdf("/mnt/efs/marta.pinedamoncusi//CCU037/CCU037_01/OUTPUT_diferences_NAvsAZ_gdppr/Prevalence_ethnicities_from_linkedHESvsGDPPR.pdf",width=10.06, height=8.06,onefile=T)  
 print(p)
 print(p2)
dev.off()
