...
#'subset with individuals present in Freya's table:   
  #ref_ID <- dbGetQuery(con,"SELECT distinct(NHS_NUMBER_DEID) FROM .ccu037_gdppr_primary_and_snomed_fa")
  #mydata = inner_join(saved,ref_ID,  by = "NHS_NUMBER_DEID")
  
  
#'Include the columns with snomed codes from ".ccu037_gdppr_skinny_patient_char_pcandsnomedcode" columns ETHNIC, SNOMED_conceptId_description, PrimaryCode 
  ...
  eth_codes<- dbGetQuery(con,"SELECT PERSON_ID, ETHNIC, SNOMED_conceptId_description, PrimaryCode FROM .ccu037_gdppr_skinny_patient_char_pcandsnomedcode")
