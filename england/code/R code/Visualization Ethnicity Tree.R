...
ggraph(mygraph, layout = 'dendrogram', circular = FALSE) +
        geom_node_text(aes(label=vertices$group, filter=leaf, color=cluster),size=1.2, angle=90, hjust=1, nudge_y = -0.01) + #text
        ylim(-4.9, NA) +
        theme(legend.position="none") +
        geom_edge_diagonal() #geom_edge_link() #for straight line

ggsave("C:/Users/martapm/Desktop/temp.tiff", width = 45, height = 10, units = "in" , dpi=780)
...
mygraph <- graph_from_data_frame(edges, vertices=vertices)
ggraph(mygraph, layout = 'dendrogram', circular = FALSE) +
        geom_node_text(aes(label=temp, filter=leaf, color=cluster),size=1.6, angle=90, hjust=1, nudge_y = -0.01) + #text
        ylim(-7.9, NA) +
        theme(legend.position="none") +
        geom_edge_diagonal() #geom_edge_link() #for straight line
